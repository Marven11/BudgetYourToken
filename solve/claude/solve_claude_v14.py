...
def _precompute_simulation():
    ...
    no_cache_output_len = len("Example output of matching no cache")
...
def _dp_segment_with_lookahead(tokens, system_len):
    n = len(tokens)
    if n == 0:
        return set()
...
def _compute_optimal_writes(rounds_info):
    ...
    for seg_start, seg_end in segments:
        ...
        writes = _dp_segment_with_lookahead(tokens, system_len)
