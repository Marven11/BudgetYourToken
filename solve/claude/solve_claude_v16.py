...
def _dp_segment(tokens, system_len, is_first_segment):
    n = len(tokens)
    if n == 0:
        return set()
...
def _compute_optimal_writes(rounds_info):
    ...
    for idx, (seg_start, seg_end) in enumerate(segments):
        ...
        writes = _dp_segment(tokens, system_len, idx == 0)
...
def optimizer(history: list[Message]) -> list[Message]:
    ...
    if n < _state.prev_len:
        ...
        if _state.system_cached:
            _state.known_caches.append([_msg_key(history[0])])
