...
def _dp_segment_multi(tokens, system_len):
    n = len(tokens)
    if n == 0:
        return set()
    ...
    for w in range(n):
        ...
        cost += system_len * CACHED_RATE + (tokens[w] - system_len) * INPUT_RATE
    ...
    for w in range(n):
        ...
        for w2 in range(w + 1, n):
            cost = dp[w]
            for i in range(w + 1, w2):
                cost += tokens[w] * CACHED_RATE + (tokens[i] - tokens[w]) * INPUT_RATE
            cost += tokens[w] * CACHED_RATE + (tokens[w2] - tokens[w]) * INPUT_RATE
...
def _compute_optimal_writes(rounds_info):
    ...
    for seg_start, seg_end in segments:
        ...
        writes = _dp_segment_multi(tokens, system_len)
        for w in writes:
