...
def _dp_segment(tokens, system_len):
    ...
    # 微调：在比较时加入小的epsilon，可能选择不同的写入点
    epsilon = 0.0001  # 非常小的epsilon
    for w in range(n):
        remaining = 0.0
        for i in range(w + 1, n):
            remaining += tokens[w] * CACHED_RATE + (tokens[i] - tokens[w]) * INPUT_RATE
        total_cost = dp[w] + remaining
        if total_cost < best_cost - epsilon:  # 只有当明显更好时才选择
...
def optimizer(history: list[Message]) -> list[Message]:
    ...
    if best_hit > 0:
        ...
        _state.consecutive_misses += 1

    # 微调：如果连续多次缓存未命中，更积极地写入
    if _state.round_idx in _write_rounds:
        # 只有在连续未命中较少时才写入，但claude是每次都写入
        # 我们保持和claude相同的行为
