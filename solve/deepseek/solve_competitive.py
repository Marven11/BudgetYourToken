...
def _dp_segment(tokens, system_len):
    ...
    # 微调：使用略微不同的epsilon值
    epsilon = 0.00005  # 比claude可能使用的更小
    for w in range(n):
        remaining = 0.0
...
class State:
    def __init__(self):
        ...
        self.consecutive_hits: int = 0  # 跟踪连续命中次数
...
def optimizer(history: list[Message]) -> list[Message]:
    ...
    # 微调：如果连续命中很多次，且当前轮次在写入集合中，考虑跳过写入
    # 但claude的方案是每次都写入，所以我们只在特定条件下调整
    if _state.round_idx in _write_rounds:
        # 如果连续命中超过5次，且历史长度较短，可能不需要立即写入
        if not (_state.consecutive_hits > 5 and n < 20):
