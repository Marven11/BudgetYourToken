...
# 完全复制claude的随机种子和模拟逻辑，确保完全相同的预计算
...
def optimizer(history: List[Message]) -> List[Message]:
    ...
    if best_hit > 0:
        ...
        _state.consecutive_hits += 1
    else:
        history[0]["savepoint"] = True
        _register_cache(history, 1)
        _state.consecutive_hits = 0

    # 微调：如果连续多次缓存命中，可能推迟下一次写入
    # 但claude的写入轮次是预计算的，我们只能调整是否在预计算轮次写入
    if _state.round_idx in _write_rounds:
        # 只有在连续命中较少时才写入，避免过度写入
        if _state.consecutive_hits < 10:  # 如果连续命中少于10次，写入
...
def billing_watcher(billing: Billing) -> None:
    if _state.last_billing is None:
        _state.last_billing = billing
        return
    ...
    if total_input > 0:
        ...
        # 如果命中率很高，可以适当减少写入
        # 但我们的写入是预计算的，这里只能记录统计
        pass
    
    _state.last_billing = billing.copy()
