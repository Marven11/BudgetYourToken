...
def _dp_segment(tokens, system_len):
    ...
    # 调整：稍微增加写入成本，减少不必要的写入
    adjusted_write_rate = WRITE_RATE * 1.01  # 增加1%
    
    for w in range(n):
        cost = 0.0
        for i in range(w + 1):
            cost += system_len * CACHED_RATE + (tokens[i] - system_len) * INPUT_RATE
        cost += tokens[w] * adjusted_write_rate  # 使用调整后的写入成本
        dp[w] = cost
        dp_parent[w] = -1
    ...
    for w in range(n):
        ...
        for w2 in range(w + 1, n):
            ...
            cost += tokens[w2] * adjusted_write_rate  # 使用调整后的写入成本
            if cost < dp[w2]:
                dp[w2] = cost
                dp_parent[w2] = w
    ...
    # 使用更严格的比较：只有当明显更好时才选择写入
    epsilon = 0.001  # 比之前的0.0001更大
    for w in range(n):
        remaining = 0.0
