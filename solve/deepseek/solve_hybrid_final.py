...
def _dp_segment(tokens, system_len):
    ...
    epsilon = 0.00001  # 非常小的epsilon，尝试获得略微不同的写入点
    for w in range(n):
        remaining = 0.0
...
class State:
    def __init__(self):
        ...
        self.cleaned: bool = False  # 标记是否刚发生清理
...
def optimizer(history: list[Message]) -> list[Message]:
    ...
    # 如果刚发生清理，在后半部分选择一个短消息作为额外savepoint（模仿kimi）
    # 但确保不超过4个savepoint
    if _state.cleaned and n > 1 and savepoint_count < 4:
        # 跳过系统消息，在后50%中选择最短的消息
        start_idx = max(1, n // 2)
        candidates = [(i, len(history[i]["content"])) for i in range(start_idx, n)]
        if candidates:
            candidates.sort(key=lambda x: (x[1], -x[0]))  # 长度升序，索引降序
            idx, _ = candidates[0]
            if not history[idx]["savepoint"]:
                history[idx]["savepoint"] = True
                savepoint_count += 1
    
    # DP选择的写入轮次
    if _state.round_idx in _write_rounds and n > 0 and savepoint_count < 4:
        if not history[n - 1]["savepoint"]:
