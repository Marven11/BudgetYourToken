...
def optimizer(history: List[Message]) -> List[Message]:
    global _KNOWN_CACHES, _LAST_LEN, _ROUND_IDX, _CLEANED
    ...
    # 如果刚刚清理过，在清理后的历史中选择一个短消息作为额外savepoint（模仿kimi）
    if _CLEANED and cur_len > 1 and savepoint_count < 4:
        # 选择最短的消息（跳过系统消息）
    ...
    # DP选择的写入轮次
    if _ROUND_IDX in _WRITE_ROUNDS and cur_len > 0 and savepoint_count < 4:
        if not optimized[-1]["savepoint"]:
